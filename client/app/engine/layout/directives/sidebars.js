(function() {
    ...
    angular.module('app.engine.layout')
        ...
        .directive('ctSticky', ctSticky);

    sidebar.$inject = ['$document', '$rootScope'];
    appAside.$inject = ['$window', '$rootScope', '$timeout', 'APP_MEDIAQUERY'];
    sidebarToggler.$inject = ['$window', '$timeout'];
    ctSticky.$inject = ['$window', '$timeout'];
                ...
                elem.on(eventObject, 'nav a', function (e) {
                    ...
                    if (!_this.parent().hasClass('hover') && !_this.closest('ul').hasClass('sub-menu')) {
                        ...
                        if ($('#app').hasClass('lyt-3')) {
                            space = $('#sidebar > .sidebar-container').position().top -
                                    $('header').outerHeight() + _this.position().top;
                        }

                        var offset = $('#sidebar > .sidebar-container > div').position()
                                .top + $('.nav-user-wrapper')
                                .outerHeight() + $('header').outerHeight();
                        var itemTop = isSidebarFixed() && !isBoxedPage() ? _this.parent().position().top + offset +
                                space : (_this.parent().offset().top - $('header').outerHeight());
                        menuTitle.css({
                        ...
                        if (_this.next().is('ul')) {
                            ...
                            if (_this.parent().position().top + _this.outerHeight() +
                                    offset + ul.height() > $win.height() && isSidebarFixed() && !isBoxedPage()) {
                                ul.css('bottom', 0);
                            } else {
                                ul.css('bottom', 'auto');
                            }
     ...
     * sidebarToggler
     */
    function sidebarToggler($window, $timeout) {
        return {
            restrict: 'C',
            ...
            link: function (scope, elem, attrs) {
                elem.on('click', function () {
                    $('.main-content')
                        .on('webkitTransitionEnd mozTransitionEnd oTransitionEnd otransitionend transitionend',
                            function () {
                                ...
                                $('.main-content')
                                    .off('webkitTransitionEnd mozTransitionEnd ' +
                                        'oTransitionEnd otransitionend transitionend');
                            });
     ...
     * ctSticky
     */
    function ctSticky($window, $timeout) {
        return {
            restrict: 'A',
            scope: {
                ctStickyDisabled: '&'
